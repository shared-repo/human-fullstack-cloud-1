...
public class EchoServer {
	...
	public static void main(String[] args) {
		...
		try {
			// 소켓만들기 + bind + listen
			listener = new ServerSocket(9900, 100); // port, 대기열크기
			
			while (true) {
				// 반환된 Socket 객체는 연결된 클라이언트와 통신할 수 있는 소켓
				...
				PrintWriter pw = null;
				try {
					is = socket.getInputStream(); // 네트워크와 연결된 데이터 수신 도구 (읽기 도구)
					isr = new InputStreamReader(is);
					br = new BufferedReader(isr);
					String message = br.readLine();	// 클라이언트가 송신한 데이터 읽기
					System.out.printf("[%s] : %s\n", socket.getRemoteSocketAddress(), message);
					
					os = socket.getOutputStream();// 네트워크와 연결된 데이터 송신 도구 (쓰기 도구)
